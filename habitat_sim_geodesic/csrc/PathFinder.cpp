...
struct PathFinder::Impl {
 ...
 private:
  ...
  std::tuple<float, std::vector<vec3f>> findPathInternal(const vec3f& start,
                                                         dtPolyRef startRef,
...
bool PathFinder::Impl::findPath(MultiGoalShortestPath& path) {
  ...
  for (size_t i = 0; i < path.requestedEnds.size(); ++i) {
...
T PathFinder::Impl::tryStep(const T& start, const T& end, bool allowSliding) {
  ...
  if (!this->islandSystem_->hasConnection(startRef, endRef)) {
    ...
    // shared between two different connected components (aka infinitely
    // thin walls) The way to deal with this is to nudge the point into the
    // polygon we want it to be 'moveAlongSurface' tells us which polygon we
    // want endPoint to be in through the polys list
    const dtMeshTile* tile = 0;
    const dtPoly* poly = 0;
